...
class ArticleServiceTest {
    ...
    @Captor
    ArgumentCaptor<Iterable<Article>> articlesCaptor;
    ...
    @Test
    void search_shouldDelegateToRepositoryWithCorrectSpecification() {
        var req = new ArticleSearchRequest("author", "title", "tag", "2025-07", 0, 5);
        Pageable pageable = PageRequest.of(0, 10);
        Page<Article> result = new PageImpl<>(List.of());

        when(articleRepository.findAll(any(Specification.class), eq(pageable))).thenReturn(result);

        var page = articleService.search(req, pageable);
    ...
    void fetchAndSaveArticles_shouldFallbackToSaveOneByOneOnIntegrityViolation() {
        ...
        doThrow(new DataIntegrityViolationException("unique constraint"))
                .when(articleRepository).saveAll(any());
