...
class ArticleControllerTest {
  ...
  public static final String PATH = "/api/v1/articles";
  ...
  void searchArticles_shouldReturn400IfMonthIsInvalid() throws Exception {
    mockMvc
        .perform(get(PATH).param("month", "2025-07"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.status").value(400))
        .andExpect(jsonPath("$.errors[0].field").value("month"))
        .andExpect(
            jsonPath("$.errors[0].message")
                .value("Invalid month. Must be in English (e.g. 'july')"));
  }
  ...
  void deleteArticle_shouldReturn204IfDeleted() throws Exception {
    ...
    mockMvc.perform(delete(PATH + "/123")).andExpect(status().isNoContent());
  }
  ...
  void deleteArticle_shouldReturn404IfNotFound() throws Exception {
    ...
    mockMvc.perform(delete(PATH + "/not-found")).andExpect(status().isNotFound());
  }
}
